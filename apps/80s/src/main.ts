...
function play() {
  const resource = createAudioResource(process.env.DISCORD_80S_STREAM_URI, {
...
async function connectToChannel(channel: VoiceBasedChannel) {
  const channelId = process.env.DISCORD_80S_CHANNEL_ID;
...
async function bootstrap() {
  ...
  const rest = new REST({ version: "10" }).setToken(
    process.env.DISCORD_80S_TOKEN
  );

  try {
    console.log("Started refreshing application (/) commands.");

    await rest.put(Routes.applicationCommands(process.env.DISCORD_80S_APP_ID), {
      body: commands,
    });
  ...
  await client.login(process.env.DISCORD_80S_TOKEN);
}
